...
class self_attn(nn.Module):
    def __init__(self, in_channels, mode='hw'):
        ...
        self.mode = mode
        self.query_conv = conv(in_channels, in_channels // 4, kernel_size=(1, 1))
        self.key_conv = conv(in_channels, in_channels // 4, kernel_size=(1, 1))
...
class PAA_kernel(nn.Module):
    def __init__(self, in_channel, out_channel, receptive_size=3):
        ...
        self.conv5=conv(out_channel,out_channel,kernel_size=3)
...
class PAA_e(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.se(x_cat + self.conv_res(x)))

        return x
#
# a=PAA_e(64,4)
# w=torch.rand(1,64,224,224)
# print(a(w).shape)